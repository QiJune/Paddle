...
class ControlFlowGraph(object):
    ...
    def _shape_compare(self, x_shape, cache_shape, level=0):
        if level == 0:
            return x_shape == cache_shape
        elif level == 1:
            if x_shape[0] == -1 and cache_shape == -1:
                return abs(reduce(mul, x_shape, 1)) < abs(
                    reduce(mul, cache_shape, 1))
            return False
        else:
            return True
